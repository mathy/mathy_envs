...
def train(config: PPOConfig, init_from: Optional[str] = None) -> None:
    ...
    print(f"training environment name: {config.env_name}")
    ...
    env = gym.make(config.env_name, invalid_action_response="raise", verbose=False)
    ...
    root_path = Path("./trained") / config.env_name
    ###################### logging ######################
    ...
    print(f"current logging run number for {config.env_name}: {run_num}")
...
def test(config: PPOConfig, checkpoint_path: str):
    ...
    env = gym.make(config.env_name, verbose=True)
